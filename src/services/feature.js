...
const handleRenewCertification = async (email, certificateNumber, _expirationDate) => {
    ...
    try {
        ...
        if (isNumberExist) {
            ...
            var certificateIndex = isNumberExistInBatch.batchId - 1;

            try {
                // Verify certificate on blockchain
                const isPaused = await newContract.paused();
                const issuerAuthorized = await newContract.hasRole(process.env.ISSUER_ROLE, idExist.issuerId);
                const verifyOnChain = await newContract.verifyBatchRoot(certificateIndex);
                ...
                if (verifyOnChain[0] === false) {
                    ...
                    try {
                        ...
                        try {
                            ...
                            console.log("Data2", certificateData);
                            // Insert certification status data into database
                    ...
                    // Common batch expiration
                    return ({ code: 400, status: "FAILED", message: messageCode.msgCommonBatchExpiration });
                }
